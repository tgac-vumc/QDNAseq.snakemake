...
SettingService = [
        ...
        expand(DIR_OUT + "{binSize}kbp/BED/{sample}_Cosmic.bed", binSize=BINSIZES ,sample=SAMPLES.keys()),
        expand(DIR_OUT + "qc-fastq/{sample}_fastqc.html", sample=wholenames),
        expand(DIR_OUT + "qc-bam/{sample}_fastqc.html", sample=SAMPLES.keys()),
...
rule bwa_aln:
    ...
    output:
        sai=temp(DIR_OUT + "bam/{sample}.sai"),
        samse=temp(DIR_OUT + "bam/{sample}.samse.sam")
    params:
        ref= config['all']['REF'],
        n=config['bwa']['max_edit_distance'],
        q=config['bwa']['read_trimming_param'],
    threads: config['all']['THREADS']
    log: DIR_OUT + "logs/bwa/{sample}.log"
    shell:
...
rule samtools_sort:
    input:
        samse=DIR_OUT + "bam/{sample}.samse.sam",
	    sai=DIR_OUT + "bam/{sample}.sai"
    output:
        temp(DIR_OUT + "tmp/{sample}.all.bam")
    params:
        output=DIR_OUT + "tmp/{sample}.all"
    log: DIR_OUT + "logs/samtools/{sample}.log"
    shell:
...
rule mark_duplicates:
    ...
    output:
        bam=DIR_OUT + "bam/{sample}.bam",
        bai=DIR_OUT + "bam/{sample}.bai",
        bambai=DIR_OUT + "bam/{sample}.bam.bai",
        metrics_file=DIR_OUT + "stats/{sample}.md.metrics"
    log: DIR_OUT + "logs/mark_duplicates/{sample}.log"
    shell:
...
rule generate_stats:
    ...
    output:
        DIR_OUT + "stats/{sample}.reads.all"
    params:
        outdir=DIR_OUT + "stats"
    shell:
        "{input.script} {input.bam} {wildcards.sample} {params.outdir} {output}"

rule QDNAseq_binReadCounts:
    input:
        bams=expand(DIR_OUT + "bam/{sample}.bam", sample=SAMPLES.keys()),
    output:
    ...
    log: DIR_OUT + "{binSize}kbp/logs/binReadCounts.log"
    script:
...
rule QDNAseq_normalize:
    ...
    params:
        ...
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{binSize}kbp/logs/normalizeBins.log"
    script:
...
rule deWave:
    ...
    params:
        ...
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{binSize}kbp/logs/dewave.log"
    script:
        "scripts/deWave.R"
...
rule QDNAseq_segment:
    ...
    params:
        ...
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{binSize}kbp/logs/segment.log"
    script:
...
rule CNA_call:
    ...
    params:
        ...
        failed=DIR_OUT + "{binSize}kbp/logs/failed_samples.txt",
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{binSize}kbp/logs/call.log"
    script:
        "scripts/CNA_call.R"
...
rule CNA_call_cellularity_based:
    ...
    params:
        ...
        failed=DIR_OUT + "{ACEbinSize}kbp/logs/failed_samples.txt",
        minimum_cellularity=config["QDNAseq"]["minimum_cellularity"],
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{ACEbinSize}kbp/logs/CNA_call_cellularity_based.log"
    script:
...
rule CNA_bedfiles:
    ...
    params:
        ...
        failed=DIR_OUT + "{ACEbinSize}kbp/logs/failed_samples.txt",
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{ACEbinSize}kbp/logs/bedfiles.log"
    script:
...
rule annotate_focalCNA:
 ...
 #TODO remove from addCosmicCencus census location: /net/nfs/PAT/home/matias/data/ref/cosmic/hg19_okt2015/CosmicMutantExport.tsv
    ...
    output:
        DIR_OUT + "{binSize}kbp/BED/{sample}_Cosmic.bed"
    params:
        outdir=DIR_OUT + "{binSize}kbp/BED/"
    log: DIR_OUT + "{binSize}kbp/logs/{sample}_annotate_focalCNA.log"
    shell:
...
#TODO change reCalled to normal calls or cellularity based
    ...
    params:
        ...
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{ACEbinSize}kbp/logs/CGHregions.log"
    script:
...
rule makeCGHregionsTable:
    ...
    params:
        ...
        cytobands=config["CGHregions"]["cytobands"],
        suppressMessages=config["all"]["suppressMessages"]
    log: DIR_OUT + "{binSize}kbp/logs/CGHregionstable.log"
    script:
...
rule annotate_RegionsFocalCNAbed:
    ...
    log: DIR_OUT + "{binSize}kbp/logs/annotatateregions.log"
    shell:
...
#TODO script lane-summary does contain relative links to files - maybe better to change
    input:
        stats=expand(DIR_OUT + "stats/{sample}.reads.all", sample=SAMPLES.keys()),
        index=expand(DIR_OUT + "{{binSize}}kbp/profiles/{profiletype}/index.html", profiletype=profiletypes),
        script='scripts/lane-summary.sh',
        qcfastq=expand(DIR_OUT + "qc-fastq/{wholename}_fastqc.html", wholename=wholenames),
        bamqc=expand(DIR_OUT + "qc-bam/{sample}_fastqc.html", sample=SAMPLES.keys()),
    output:
        DIR_OUT + "{binSize}kbp/summary.html"
    params:
        bamfolder=DIR_OUT + "bam/",
    shell:
...
rule qcfastq:
    ...
    output:
        qcfastq=DIR_OUT + "qc-fastq/{sample}_fastqc.html",
        qczip=temp(DIR_OUT + "qc-fastq/{sample}_fastqc.zip")
        #qcfastq=expand(DIR_OUT + "qc-fastq/{{sample}}.fastqc.html", sample=SAMPLES.keys()),
        #qczip=temp(expand(DIR_OUT + "qc-fastq/{{sample}}_fastqc.zip", sample=SAMPLES.keys()))
    threads: config['all']['THREADS']
    params:
        qcfastqdir=DIR_OUT + "qc-fastq/"
    log: DIR_OUT + "logs/qc-fastq/{sample}.log"
    shell:
...
rule qcbam:
    input:
        bam=DIR_OUT + "bam/{sample}.bam",
    output:
        bamqc=DIR_OUT + "qc-bam/{sample}_fastqc.html",
        bamqczip=temp(DIR_OUT + "qc-bam/{sample}_fastqc.zip"),
    threads: config['all']['THREADS']
    params:
        qcbamdir=DIR_OUT + "qc-bam/"
    log: DIR_OUT + "logs/qc-bam/{sample}.log"
    shell:
...
rule ACE:
    ...
    params:
        ...
        failed=DIR_OUT + "{ACEbinSize}kbp/logs/failed_samples.txt",
        suppressMessages=config["all"]["suppressMessages"]
    log:DIR_OUT + "{ACEbinSize}kbp/ACE/{ploidy}N/log.tsv"
    script:
        "scripts/Run_ACE.R"
...
rule postanalysisloop_ACE:
    ...
    params:
        ...
        failed=DIR_OUT + "{ACEbinSize}kbp/logs/failed_samples.txt",
        suppressMessages=config["all"]["suppressMessages"]
    log:DIR_OUT + "{ACEbinSize}kbp/ACE/{ploidy}N_log.tsv"
    script:
